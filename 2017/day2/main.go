...
func main() {
	in := [][]int{{4347, 3350, 196, 162, 233, 4932, 4419, 3485, 4509, 4287, 4433, 4033, 207, 3682, 2193, 4223},
	...
	fmt.Println(CheckSum(in))
	// fmt.Println(filter([]int{1, 2, 3, 4, 5, 6}, evens))
	fmt.Println(CheckSum2(in))
}

func evens(f int) bool { return f%2 == 0 }
func CheckSum(arr [][]int) (sum int) {
	for _, row := range arr {
		if len(row) < 2 {
			continue
		}
...
func CheckSum2(arr [][]int) (sum int) {
	for _, row := range arr {
		if len(row) < 2 {
			continue
		}
		evens := commonFactors(row)
		sort.Ints(evens)
		fmt.Println(evens)
		min := evens[0]
		max := evens[len(evens)-1]
		sum += max / min
	}
	return
}
...
func commonFactors(vs []int) []int {
	f := func(v int, vs []int) bool {
		for _, i := range vs {
			xy := float64(v) / float64(i)
			yx := float64(i) / float64(v)
			// fmt.Println(v, i, xy, yx, int(xy), int(yx))
			if math.Floor(xy) == xy && xy != 0 && i != v {
				return true
			}
			if math.Floor(yx) == yx && yx != 0 && i != v {
				return true
			}
